...
[+] 修复动态分段拆分时任务误判完成/过早进入下一轮重试的问题，避免仍在下载的分段被主流程跳过 (commit: a61e3ae)
...
[+] 桌面端统一接入 Chromium 风格平滑滚动，覆盖设置/日志/状态/关于/更新等页面 (commit: e823111)
[+] 通知卡片支持展开长标题/长消息，悬停或展开时自动暂停倒计时 (commit: e823111)
[+] 关于页、日志页、性能监视器与多处调试页面重构，信息层级与可读性提升 (commit: e823111)
...
[+] 新增按语言/脚本分组的字体设置系统，支持英文、简中、繁中及更多语言分别配置字体 (commit: 13d6cdb)
[+] 新增字体覆盖检测与语言感知字体选择器，自定义字体可按语言单独绑定 (commit: 13d6cdb)
[+] 性能监视器新增应用 CPU / 内存 / 峰值内存采样 (commit: e823111)
...
### 构建与开发体验:
[+] 新增 Windows 下 rhttp 构建补丁脚本，自动定位 Pub Cache 中的实际包路径并修补构建脚本 (commit: d868826)
[+] README_CN 增加补丁脚本使用说明，pubspec.yaml 补充 Flutter 国内镜像环境变量提示 (commit: d868826)
...
[+] 语言包新增 @@fontLocale / @@defaultFont / @@defaultEnglishFont 元数据，支持声明字体类别与默认字体 (commit: 13d6cdb)
[+] 新增繁体中文示例语言包，并扩展喵喵语示例演示字体继承关系 (commit: 13d6cdb)
